...
namespace Ninject.Extensions.Conventions.BindingGenerators
{
	...
	/// </summary>
	public class AllBaseBindingGenerator : IBindingGenerator
	{
		/// <summary>
		...
		private IEnumerable<IBindingWhenInNamedWithOrOnSyntax<object>> RecursivelyBindToAbstractBaseTypes(Type type, IBindingRoot bindingRoot)
		{
			...
			if (baseType != null)
			{
				...
				{
					var abstractType = baseType.BaseType;
					while (abstractType != null && abstractType.IsAbstract)
					{
						bindings.Add(bindingRoot.Bind(abstractType).To(type));
						abstractType = abstractType.BaseType;
					}
				}
			}
			return bindings;
